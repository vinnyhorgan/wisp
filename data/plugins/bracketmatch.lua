...
local function update_state(line_limit)
    ...
    local doc = getmetatable(core.active_view) == DocView and core.active_view.doc
    if not doc then
        state = {}
        return
    end
    ...
    if
        ...
        and state.line == line
        and state.col == col
        and state.change_id == change_id
        and state.limit == line_limit
    then
        return
    end
    ...
    for _, map in ipairs(bracket_maps) do
        for i = 0, -1, -1 do
            ...
            if close then
                line2, col2 =
                    get_matching_bracket(doc, line, col, line_limit, open, close, map.step)
                goto found
            end
        end
    end
...
function DocView:draw_line_text(idx, x, y)
    ...
    -- CommandView extends DocView; underlining a bracket in the find
    -- prompt is noise
    ...
    if self.doc == state.doc and idx == state.line2 then
        ...
        renderer.draw_rect(x1, y + self:get_line_height() - h, x2 - x1, h, style.bracketmatch)
    end
end
