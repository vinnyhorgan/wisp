-- bookmarks: a mark in the gutter, and a key that walks them.
-- original implementation by petri häkkinen, for lite.
--
-- the part worth keeping is `shift_lines`: markers are line numbers,
-- and line numbers move when you edit above them, so the plugin hooks
-- the doc's raw insert and remove to carry them along. that is the bit
-- everyone gets wrong when they write this themselves
...
local function shift_lines(doc, at, diff)
    ...
    for line in pairs(cache[doc]) do
        -- lua 5.5 for-loop variables are const, so the shifted number
        -- is a local of its own rather than an assignment back
        local shifted = line >= at and line + diff or line
        t[shifted] = true
    end
    cache[doc] = t
end
...
function DocView:draw_line_gutter(idx, x, y)
    -- CommandView extends DocView and has no gutter worth marking
    if getmetatable(self) == DocView and cache[self.doc] and cache[self.doc][idx] then
        local h = self:get_line_height()
        renderer.draw_rect(x, y, style.padding.x * 0.4, h, style.marker)
    end
